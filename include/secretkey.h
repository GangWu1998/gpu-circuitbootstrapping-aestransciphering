...
public:
    ...
    explicit PhantomRelinKey(const PhantomContext &context, const lbcrypto::EvalKey<lbcrypto::DCRTPoly>& openfhe_swk);
    ...
    {
        if (gen_flag_)
            throw std::invalid_argument("PhantomRelinKey has already been generated");
...
private:
    bool gen_flag_ = false;
    std::map<size_t, PhantomRelinKey> relin_keys_;
...
public:
    ...
    [[nodiscard]] auto& get_relin_keys(size_t elt) const
    {
        return relin_keys_.at(elt);
    }
    ...
    void save(std::ostream& stream) const
    {
        ...
        for (const auto& [elt, key] : relin_keys_)
        {
            stream.write(reinterpret_cast<const char*>(&elt), sizeof(std::size_t));
            key.save(stream);
        }
    }
    ...
    {
        if (gen_flag_)
            throw std::invalid_argument("PhantomGaloisKey has already been generated");
        ...
        {
            size_t elt;
            stream.read(reinterpret_cast<char*>(&elt), sizeof(std::size_t));
            PhantomRelinKey key;
            key.load(stream);
            relin_keys_.insert(std::make_pair(elt, std::move(key)));
        }
