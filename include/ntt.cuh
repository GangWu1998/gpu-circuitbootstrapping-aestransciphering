...
public:
    ...
    explicit DNTTTable(const std::vector<phantom::arith::Modulus> &h_modulus, const size_t dim, const cudaStream_t &s)
    {
        const size_t log_dim = phantom::arith::get_power_of_two(dim);
        const size_t batch_size = h_modulus.size();
...
// Out-of-place inverse NTT: reads from in (evaluation domain) and writes to out (coefficient domain).
...
// Permute input (evaluation-domain automorphism map) and then run inverse NTT into out (coefficient domain).
// This avoids an extra global-memory permute pass when the next stage is inwt anyway.
...
// Batched single-modulus NTT: one modulus/twiddle table shared across all batches.
void fnwt_1d_opt_batched(uint64_t *inout, const uint64_t *twiddles, const uint64_t *twiddles_shoup, const DModulus *modulus,
                         size_t dim, size_t batch, const cudaStream_t &stream);

void inwt_1d_opt_batched(uint64_t *inout, const uint64_t *itwiddles, const uint64_t *itwiddles_shoup, const DModulus *modulus,
                         const uint64_t *scalar, const uint64_t *scalar_shoup, size_t dim, size_t batch, const cudaStream_t &stream);
