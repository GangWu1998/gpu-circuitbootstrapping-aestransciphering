...
namespace phantom::util {
    ...
    [[nodiscard]] inline uint32_t get_elt_from_step(int step, size_t coeff_count) {
        const uint32_t n = coeff_count;
    ...
    private:
        ...
        std::map<size_t, cuda_auto_ptr<uint32_t>> permutation_tables_;
        std::map<size_t, cuda_auto_ptr<uint64_t>> index_raw_tables_; // only used by bfv
        bool is_bfv_;
    ...
    public:
        ...
        void insert_permutation_table(const size_t index, const cudaStream_t &stream)
        {
            if (permutation_tables_.find(index) != permutation_tables_.end())
                return; // already exists
            auto permutation_table = phantom::util::make_cuda_auto_ptr<uint32_t>(coeff_count_, stream);
            ...
            for (size_t i = coeff_count_; i < coeff_count_ << 1; i++) {
                const uint32_t reversed = phantom::arith::reverse_bits(static_cast<uint32_t>(i), coeff_count_power_ + 1);
                uint64_t index_raw = (static_cast<uint64_t>(index) * static_cast<uint64_t>(reversed)) >> 1;
                index_raw &= static_cast<uint64_t>(coeff_count_minus_one);
                u32temp[i - coeff_count_] = phantom::arith::reverse_bits(static_cast<uint32_t>(index_raw), coeff_count_power_);
            }
            cudaMemcpyAsync(permutation_table.get(), u32temp.data(), coeff_count_ * sizeof(uint32_t),
                            cudaMemcpyHostToDevice, stream);
            permutation_tables_.insert(std::pair(index, std::move(permutation_table)));
        }
        ...
        explicit PhantomGaloisTool(const std::vector<uint32_t> &galois_elts, int coeff_count_power,
            ...
            for (const auto &galois_elt: galois_elts_) {
                insert_permutation_table(galois_elt, stream);
            }

            if (is_bfv_) {
                for (const auto &galois_elt: galois_elts_)
                {
                    auto index_raw_table = phantom::util::make_cuda_auto_ptr<uint64_t>(coeff_count_, stream);
                    ...
                    cudaMemcpyAsync(index_raw_table.get(), u64temp.data(), coeff_count_ * sizeof(uint64_t),
                                    cudaMemcpyHostToDevice, stream);
                    index_raw_tables_.insert(std::pair(galois_elt, std::move(index_raw_table)));
                }
            }
        }
        ...
        void apply_galois(uint64_t *operand, const DNTTTable &rns_table, size_t coeff_mod_size, size_t galois_elt,
                          uint64_t *result, const cudaStream_t &stream);

        void apply_galois_ntt(uint64_t *operand, size_t coeff_mod_size, size_t galois_elt, uint64_t *result,
