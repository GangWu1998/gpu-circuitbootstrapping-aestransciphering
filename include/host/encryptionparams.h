...
namespace phantom {
    ...
    public:
        ...
        explicit EncryptionParameters(scheme_type scheme = scheme_type::scheme_none) : scheme_(scheme) {
            // set default mul tech for BFV
            if (scheme_ == scheme_type::bfv)
                mul_tech_ = mul_tech_type::hps;
            else
                mul_tech_ = mul_tech_type::mul_tech_none;
        }
        ...
        */
        inline void set_poly_modulus_degree(std::size_t poly_modulus_degree) {
            if (scheme_ == scheme_type::scheme_none && poly_modulus_degree) {
        ...
        */
        inline void set_special_modulus_size(std::size_t special_modulus_size) {
            if (scheme_ == scheme_type::scheme_none && special_modulus_size) {
        ...
        */
        inline void set_coeff_modulus(const std::vector<arith::Modulus> &coeff_modulus) {
            ...
            if (scheme_ == scheme_type::scheme_none) {
    ...
    private:
        [[nodiscard]] static bool is_valid_scheme(std::uint8_t scheme) noexcept {
            switch (scheme) {
                case static_cast<std::uint8_t>(scheme_type::scheme_none):
