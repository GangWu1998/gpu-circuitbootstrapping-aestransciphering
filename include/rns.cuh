...
namespace phantom {
    ...
    public:
        ...
        void divide_and_round_q_last(uint64_t *dst, const uint64_t *src, size_t cipher_size,
                                     const cudaStream_t &stream) const;

        void divide_and_round_q_last_ntt(uint64_t *dst, uint64_t *src, size_t cipher_size,
                                         const DNTTTable &rns_tables, const cudaStream_t &stream) const;

        void divide_and_round_q_last_ntt_multiple_levels(
            uint64_t* dst, const uint64_t* src, size_t cipher_size, size_t levels,
            const DNTTTable& rns_tables, const cudaStream_t& stream) const;

        void mod_t_and_divide_q_last_ntt(uint64_t *dst, uint64_t *src, size_t cipher_size,
                                         const DNTTTable &rns_tables, const cudaStream_t &stream) const;
