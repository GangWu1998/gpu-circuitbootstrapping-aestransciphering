                                                        ...
                                                        size_t chain_index, const cudaStream_t& stream) const
{
    ...
    cipher.is_ntt_form_ = is_ntt_form;
    cipher.scalingFactor_ = temp_cipher.GetScalingFactor();
                                          ...
                                          PhantomCiphertext& cipher)
{
    ...
    else if (scheme == scheme_type::ckks)
    {
        ...
        cipher.scalingFactor_ = plain.GetScalingFactor();
    }
                                                ...
                                                PhantomRelinKey& relin_keys, const cudaStream_t& stream) const
{
    ...
    const size_t dnum = static_cast<size_t>(std::ceil(static_cast<double>(size_Q) / static_cast<double>(size_P)));
...
PhantomGaloisKey PhantomSecretKey::create_galois_keys(const PhantomContext& context) const
{
    ...
    for (const auto & galois_elt : galois_elts)
    {
        ...
        key_galois_tool->apply_galois_ntt(secret_key, key_mod_size, galois_elt, rotated_secret_key.get(),
                                          s);
        PhantomRelinKey relin_key;
        generate_one_kswitch_key(context, rotated_secret_key.get(), relin_key, s);
        relin_key.gen_flag_ = true;
        galois_keys.relin_keys_.insert(std::make_pair(galois_elt, std::move(relin_key)));
    }
                                    ...
                                    PhantomPlaintext& destination, const cudaStream_t& stream)
{
    ...
    destination.scalingFactor_ = encrypted.GetScalingFactor();
}
...
PhantomRelinKey::PhantomRelinKey(const PhantomContext& context,
                                 const lbcrypto::EvalKey<lbcrypto::DCRTPoly>& openfhe_swk)
{
    const auto& s = cudaStreamPerThread;

    // Extract Phantom encryption parameters
    auto& key_context_data = context.get_context_data(0);
    auto& key_parms = key_context_data.parms();
    auto& key_modulus = key_parms.coeff_modulus();
    const auto poly_degree = key_parms.poly_modulus_degree();
    ...
    public_keys_ptr_ = make_cuda_auto_ptr<uint64_t*>(dnum, s);

    const std::vector<lbcrypto::DCRTPoly>& openfhe_av = openfhe_swk->GetAVector();
    const std::vector<lbcrypto::DCRTPoly>& openfhe_bv = openfhe_swk->GetBVector();
    const size_t openfhe_swk_dcrt_size = openfhe_av.at(0).GetAllElements().size();
    for (size_t dnum_idx = 0; dnum_idx < dnum; dnum_idx++)
    {
        PhantomPublicKey pk;
        pk.prng_seed_a_ = make_cuda_auto_ptr<uint8_t>(global_variables::prng_seed_byte_count, s);
        pk.pk_.resize(2, openfhe_swk_dcrt_size, poly_degree, s);
        for (size_t i = 0; i < openfhe_swk_dcrt_size; ++i)
            cudaMemcpyAsync(pk.pk_.data() + i * poly_degree,
                            &openfhe_bv[dnum_idx].GetElementAtIndex(i).at(0), poly_degree * sizeof(uint64_t),
                            cudaMemcpyHostToDevice, s);
        for (size_t i = 0; i < openfhe_swk_dcrt_size; ++i)
            cudaMemcpyAsync(pk.pk_.data() + openfhe_swk_dcrt_size * poly_degree + i * poly_degree,
                            &openfhe_av[dnum_idx].GetElementAtIndex(i).at(0), poly_degree * sizeof(uint64_t),
                            cudaMemcpyHostToDevice, s);
        pk.gen_flag_ = true;
        public_keys_[dnum_idx] = std::move(pk);
    }

    std::vector<uint64_t*> pk_ptr(dnum);
    for (size_t twr = 0; twr < dnum; twr++)
        pk_ptr[twr] = public_keys_[twr].pk_.data();
    cudaMemcpyAsync(public_keys_ptr_.get(), pk_ptr.data(), sizeof(uint64_t*) * dnum, cudaMemcpyHostToDevice,
                    s);

    gen_flag_ = true;
}

PhantomGaloisKey::PhantomGaloisKey(const PhantomContext& context,
                                   const std::map<usint, lbcrypto::EvalKey<lbcrypto::DCRTPoly>>& openfhe_evalKeyMap)
{
    for (const auto& [elt, openfhe_evalKey] : openfhe_evalKeyMap) {
        relin_keys_.insert(std::make_pair(elt, std::move(PhantomRelinKey(context, openfhe_evalKey))));
    }
