...
// https://github.com/microsoft/SEAL/blob/3a05febe18e8a096668cd82c75190255eda5ca7d/native/src/seal/evaluator.cpp#L24
    ...
    [[nodiscard]] inline bool are_same_scale(const T &value1, const S &value2) noexcept {
        return are_close<double>(value1.GetScalingFactor(), value2.GetScalingFactor());
    }
...
// TODO: fixme
    void add_many(const PhantomContext &context, const vector <PhantomCiphertext> &encrypteds,
        ...
        destination.SetScalingFactor(encrypteds[0].GetScalingFactor());
    ...
    static void bgv_ckks_multiply(const PhantomContext &context, PhantomCiphertext &encrypted1,
        ...
        {
            encrypted1.SetScalingFactor(encrypted1.GetScalingFactor() * encrypted2.GetScalingFactor());
            encrypted1.SetNoiseScaleDeg(encrypted1.GetNoiseScaleDeg() + encrypted2.GetNoiseScaleDeg());
        }
...
// relin(encrypted1)
    ...
    static void multiply_plain_ntt(const PhantomContext &context, PhantomCiphertext &encrypted,
        ...
        double new_scale = encrypted.GetScalingFactor() * plain.GetScalingFactor();
        ...
        encrypted.SetScalingFactor(new_scale);
    }
    ...
    static void mod_switch_scale_to_next(const PhantomContext &context, const PhantomCiphertext &encrypted,
        ...
        switch (next_parms.scheme()) {
            case scheme_type::bfv:
                rns_tool.divide_and_round_q_last(destination.data(), encrypted_copy.get(), encrypted_size, stream);
                break;
            case scheme_type::bgv:
                rns_tool.mod_t_and_divide_q_last_ntt(destination.data(), encrypted_copy.get(), encrypted_size, context.gpu_rns_tables(),
                                                     stream);
                break;
            case scheme_type::ckks:
                rns_tool.divide_and_round_q_last_ntt(destination.data(), encrypted_copy.get(), encrypted_size, context.gpu_rns_tables(),
                                                     stream);
                break;
            default:
                throw invalid_argument("unsupported scheme");
        ...
        if (next_parms.scheme() == scheme_type::ckks) {
            destination.SetScalingFactor(encrypted.GetScalingFactor() / static_cast<double>(parms.coeff_modulus().back().value()));
            destination.SetNoiseScaleDeg(encrypted.GetNoiseScaleDeg() - 1);
    ...
    static void mod_switch_drop_to_next(const PhantomContext &context, const PhantomCiphertext &encrypted,
        ...
        if (&encrypted == &destination) {
            ...
            // Set other attributes
            destination.SetScalingFactor(encrypted.GetScalingFactor());
            destination.SetNoiseScaleDeg(encrypted.GetNoiseScaleDeg());
            destination.set_ntt_form(encrypted.is_ntt_form());
        }
    }
    ...
    void apply_galois_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, size_t galois_elt,
        ...
        if (parms.scheme() == scheme_type::bfv) {
            ...
            // First transform c0
            key_galois_tool->apply_galois(c0, context.gpu_rns_tables(), coeff_modulus_size, galois_elt,
                                          temp.get(),
                                          s);
            // Copy result to c0
            cudaMemcpyAsync(c0, temp.get(), coeff_modulus_size * N * sizeof(uint64_t), cudaMemcpyDeviceToDevice, s);
            // Next transform c1
            key_galois_tool->apply_galois(c1, context.gpu_rns_tables(), coeff_modulus_size, galois_elt,
                                          temp.get(),
                                          s);
        } else if (parms.scheme() == scheme_type::ckks || parms.scheme() == scheme_type::bgv) {
            // !!! DO NOT CHANGE EXECUTION ORDER!!
            // First transform c0
            key_galois_tool->apply_galois_ntt(c0, coeff_modulus_size, galois_elt, temp.get(), s);
            // Copy result to c0
            cudaMemcpyAsync(c0, temp.get(), coeff_modulus_size * N * sizeof(uint64_t), cudaMemcpyDeviceToDevice, s);
            // Next transform c1
            key_galois_tool->apply_galois_ntt(c1, coeff_modulus_size, galois_elt, temp.get(), s);
        } else {
        ...
        keyswitch_inplace(context, encrypted, temp.get(), galois_keys.get_relin_keys(galois_elt), false, s);
    }

    PhantomCiphertext eval_automorphism(const PhantomContext &context, const PhantomCiphertext &ciphertext,
                                        const uint32_t autoIndex, const PhantomGaloisKey &galois_keys)
    {
        ...
        if (parms.scheme() != scheme_type::ckks)
        {
            throw invalid_argument("eval automorphism only supported for CKKS scheme");
        }
        ...
        auto ciphertext_automorphism(ciphertext);
        auto temp_c1 = make_cuda_auto_ptr<uint64_t>(size_Ql_N, s);
        cudaMemcpyAsync(temp_c1.get(), ciphertext.data() + size_Ql_N, size_Ql_N * sizeof(uint64_t), cudaMemcpyDeviceToDevice, s);
        cudaMemsetAsync(ciphertext_automorphism.data() + size_Ql_N, 0, size_Ql_N * sizeof(uint64_t), s);
        keyswitch_inplace(context, ciphertext_automorphism, temp_c1.get(), galois_keys.get_relin_keys(autoIndex), false, s);

        PhantomCiphertext ciphertext_result(ciphertext_automorphism);
        key_galois_tool->apply_galois_ntt(ciphertext_automorphism.data(), size_Ql, autoIndex,
            ciphertext_result.data(), s);
        key_galois_tool->apply_galois_ntt(ciphertext_automorphism.data() + size_Ql_N, size_Ql, autoIndex,
            ciphertext_result.data() + size_Ql_N, s);
...
// TODO: remove recursive chain
    ...
    void hoisting_inplace(const PhantomContext &context, PhantomCiphertext &ct, const PhantomGaloisKey &glk,
        ...
        if (parms.scheme() == scheme_type::bfv) {
            key_galois_tool->apply_galois(c0.get(), context.gpu_rns_tables(), size_Ql, first_elt, acc_c0.get(),
                                          s);
        } else if (parms.scheme() == scheme_type::ckks || parms.scheme() == scheme_type::bgv) {
            key_galois_tool->apply_galois_ntt(c0.get(), size_Ql, first_elt, acc_c0.get(), s);
        } else {
            throw logic_error("scheme not implemented");
        }
        ...
        for (size_t b = 0; b < beta; b++) {
            key_galois_tool->apply_galois_ntt(modup_c1.get() + b * size_QlP_n, size_QlP, first_elt,
        ...
        key_switch_inner_prod_c2_and_evk<<<size_QlP_n / blockDimGlb.x, blockDimGlb, 0, s>>>(
                acc_cx.get(), temp_modup_c1.get(), glk.get_relin_keys(first_elt).public_keys_ptr(), modulus_QP, n,
        ...
        for (size_t i = 1; i < elts.size(); i++) {
            ...
            if (parms.scheme() == scheme_type::bfv) {
                key_galois_tool->apply_galois(c0.get(), context.gpu_rns_tables(), size_Ql, elt, temp_c0.get(), s);
            } else if (parms.scheme() == scheme_type::ckks || parms.scheme() == scheme_type::bgv) {
                key_galois_tool->apply_galois_ntt(c0.get(), size_Ql, elt, temp_c0.get(), s);
            } else {
                throw logic_error("scheme not implemented");
            }
            ...
            for (size_t b = 0; b < beta; b++) {
                key_galois_tool->apply_galois_ntt(modup_c1.get() + b * size_QlP_n, size_QlP, elt,
                                                  temp_modup_c1.get() + b * size_QlP_n, s);
            }
            ...
            key_switch_inner_prod_c2_and_evk<<<size_QlP_n / blockDimGlb.x, blockDimGlb, 0, s>>>(
                    temp_cx.get(), temp_modup_c1.get(), glk.get_relin_keys(elt).public_keys_ptr(), modulus_QP, n,
                    size_QP, size_QP_n, size_QlP, size_QlP_n, size_Q, size_Ql, beta, reduction_threshold);
