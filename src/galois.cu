...
namespace phantom::util {
    ...
    void PhantomGaloisTool::apply_galois(uint64_t *operand, const DNTTTable &rns_table, size_t coeff_mod_size,
                                         size_t galois_elt, uint64_t *result, const cudaStream_t &stream) {
        auto &index_raws = index_raw_tables_.at(galois_elt);
    ...
    void PhantomGaloisTool::apply_galois_ntt(uint64_t *operand, size_t coeff_mod_size, size_t galois_elt,
                                             uint64_t *result, const cudaStream_t &stream) {
        const auto table = permutation_tables_.at(galois_elt).get();
        uint64_t gridDimGlb = coeff_count_ * coeff_mod_size / blockDimGlb.x;
