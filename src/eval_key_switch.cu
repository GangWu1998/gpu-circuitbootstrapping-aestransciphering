...
namespace phantom {
    ...
    __global__ void key_switch_inner_prod_c2_and_evk(uint64_t *dst, const uint64_t *c2, const uint64_t *const *evks,
        ...
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < size_QlP_n; tid += blockDim.x * gridDim.x) {
            ...
            arith::uint128_t prod0, prod1;
            arith::uint128_t acc0, acc1;
            ...
            // evk[0]_a
            acc0 = arith::multiply_uint64_uint64(c2[c2_id], evks[0][evk_id]);
            // evk[0]_b
            acc1 = arith::multiply_uint64_uint64(c2[c2_id], evks[0][evk_id + size_QP_n]);
            ...
            for (uint64_t i = 1; i < beta; i++) {
                ...
                prod0 = arith::multiply_uint64_uint64(c2[c2_id + i * size_QlP_n], evks[i][evk_id]);
                add_uint128_uint128(acc0, prod0, acc0);

                prod1 = arith::multiply_uint64_uint64(c2[c2_id + i * size_QlP_n], evks[i][evk_id + size_QP_n]);
...
// cks refers to cipher to be key-switched
    void keyswitch_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, uint64_t *c2,
        ...
        auto reduction_threshold =
                (1 << (arith::bits_per_uint64 - static_cast<uint64_t>(log2(key_modulus.front().value())) - 1)) - 1;
