...
namespace phantom {
    DRNSTool::DRNSTool(size_t n, size_t size_P, const RNSBase &base, const std::vector<Modulus> &modulus_QP,
        ...
        if (!t.is_zero() && mul_tech == mul_tech_type::mul_tech_none) {
        ...
        if (mul_tech != mul_tech_type::mul_tech_none) {
     ...
     */
    void DRNSTool::divide_and_round_q_last(uint64_t *dst, const uint64_t *src, size_t cipher_size,
    ...
    __global__ void divide_and_round_reduce_q_last_kernel(
        uint64_t *dst, const uint64_t *src, const DModulus *base_q,
        const size_t n, const size_t log_n, const size_t next_base_q_size) {
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < n * next_base_q_size;
             tid += blockDim.x * gridDim.x) {
            DModulus mod = base_q[tid >> log_n];
            uint64_t c_last_coeff = src[(tid & (n - 1)) + (next_base_q_size << log_n)];
    ...
    __global__ void divide_and_round_ntt_inv_scalar_kernel(uint64_t *dst, const uint64_t *src, const DModulus *base_q,
        ...
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < poly_degree * next_base_q_size;
            ...
            uint64_t temp = sub_uint64_uint64_mod(src[tid], dst[tid], mod_value);
            dst[tid] = multiply_and_reduce_shoup(temp, q_last_inv, q_last_inv_shoup, mod_value);
        }
    }

    __global__ void divide_and_round_kernel(uint64_t* inout, const DModulus* base_q,
                                            const uint64_t* inv_q_last_mod_q,
                                            const uint64_t* inv_q_last_mod_q_shoup,
                                            const size_t n, const size_t log_n,
                                            const uint64_t next_base_q_size)
    {
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < n * next_base_q_size;
             tid += blockDim.x * gridDim.x) {
            const size_t twr = tid >> log_n;
            DModulus mod = base_q[twr];
            uint64_t mod_value = base_q[twr].value();
            uint64_t q_last_inv = inv_q_last_mod_q[twr];
            uint64_t q_last_inv_shoup = inv_q_last_mod_q_shoup[twr];

            uint64_t c_last_coeff = inout[(tid & (n - 1)) + (next_base_q_size << log_n)];

            // ci[last] mod qj
            uint64_t ci_last_mod_qj = barrett_reduce_uint64_uint64(c_last_coeff, mod.value(), mod.const_ratio()[1]);
            // ci[j] - (ci[last] mod qj)
            uint64_t temp = sub_uint64_uint64_mod(inout[tid], ci_last_mod_qj, mod_value);
            // qlast^(-1) * (ci[j] - (ci[last] mod qj)) mod qj
            inout[tid] = multiply_and_reduce_shoup(temp, q_last_inv, q_last_inv_shoup, mod_value);
        }
    }

    void DRNSTool::divide_and_round_q_last_ntt(uint64_t *dst, uint64_t *src, size_t cipher_size, const DNTTTable &rns_tables,
                                               const cudaStream_t &stream) const {
        size_t base_q_size = base_Ql_.size();
        auto next_base_q_size = base_q_size - 1;
        uint64_t gridDimGlb = n_ * next_base_q_size / blockDimGlb.x;
        size_t log_n = get_power_of_two(n_);
        ...
        for (size_t i = 0; i < cipher_size; i++) {
            ...
            divide_and_round_reduce_q_last_kernel<<<gridDimGlb, blockDimGlb, 0, stream>>>(
                    ci_out, ci_in, base_Ql_.base(), n_, log_n, next_base_q_size);
    ...
    void DRNSTool::divide_and_round_q_last_ntt_multiple_levels(
        uint64_t* dst, const uint64_t* src, const size_t cipher_size, const size_t levels,
        const DNTTTable& rns_tables, const cudaStream_t& stream) const
    {
        if (levels < 2) {
            throw std::invalid_argument("levels must be at least 2 for divide_and_round_q_last_ntt_multiple_levels");
        }

        if (base_Ql_.size() <= levels)
        {
            throw std::invalid_argument("levels must be less than the size of base_Ql_");
        }

        const size_t log_n = get_power_of_two(n_);
        const size_t final_base_q_size = base_Ql_.size() - levels;

        // temporary memory
        auto temp = make_cuda_auto_ptr<uint64_t>(n_ * base_Ql_.size(), stream);

        for (size_t i = 0; i < cipher_size; i++) {
            //  Convert ci to non-NTT form, stored in temp
            const uint64_t *ci_in = src + i * n_ * base_Ql_.size();
            nwt_2d_radix8_backward(temp.get(), ci_in, rns_tables, base_Ql_.size(), 0, stream);

            for (size_t l = 0; l < levels; l++)
            {
                const size_t base_q_size = base_Ql_.size() - l;
                const size_t next_base_q_size = base_q_size - 1;

                // qlast^(-1) * (ci[j] - (ci[last] mod qj)) mod qj
                const size_t gridDimGlb = n_ * next_base_q_size / blockDimGlb.x;
                divide_and_round_kernel<<<gridDimGlb, blockDimGlb, 0, stream>>>(
                        temp.get(), base_Ql_.base(), inv_q_last_mod_q(), inv_q_last_mod_q_shoup(), n_, log_n, next_base_q_size);
            }

            // Convert to NTT form
            uint64_t *ci_out = dst + i * n_ * final_base_q_size;
            nwt_2d_radix8_forward_inplace(temp.get(), rns_tables, final_base_q_size, 0, stream);
            cudaMemcpyAsync(ci_out, temp.get(), n_ * final_base_q_size * sizeof(uint64_t), cudaMemcpyDeviceToDevice,
                            stream);
        }
    }
    ...
    void DRNSTool::mod_t_and_divide_q_last_ntt(uint64_t *dst, uint64_t *src, size_t cipher_size,
                                               const DNTTTable &rns_tables, const cudaStream_t &stream) const {
        size_t base_q_size = base_Ql_.size();
        auto next_base_q_size = base_q_size - 1;
