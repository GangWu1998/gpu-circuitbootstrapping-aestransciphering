...
/** Cooley-Tukey butterfly using Montgomery modmul
* input: x [0, 4q), y [0, 4q)
* output: x [0, 4q), y [0, 4q)
*/
__device__ inline void ct_butterfly_mont(BasicInteger& x, BasicInteger& y,
                                         BasicInteger tw_mont,
                                         BasicInteger mod, BasicInteger mont) {
    const BasicInteger mod2 = mod << 1;
    const BasicInteger t = modMulMontLazy(y, tw_mont, mod, mont);
    x = modFast(x, mod2);
    y = x + mod2 - t;
    x += t;
}

/** Gentleman-Sande butterfly using Montgomery modmul
 * input: x [0, 2q), y [0, 2q)
 * output: x [0, 2q), y [0, 2q)
 */
__device__ inline void gs_butterfly_mont(BasicInteger& x, BasicInteger& y,
                                         BasicInteger tw_mont,
                                         BasicInteger mod, BasicInteger mont) {
    const BasicInteger mod2 = mod << 1;
...
__device__ void device_4step_ntt_forward_phase1_ws(BasicInteger s_n1n2[n1][n2 + 1],
    ...
    for (size_t n2_idx = warp_id; n2_idx < n2; n2_idx += warpsPerBlock) {
        ...
        for (size_t log_m = 0; log_m < 5; log_m++) {
            ...
            ct_butterfly_mont(left, right, s_tw_root_2n1_mont[(1 << log_m) + w_idx], mod, mont);
            reg = (1 - C) * (left - right) + right;
        }

        // elementwise multiply and transpose store into shared memory
        s_n1n2[lane_id][n2_idx] = modMulMontLazy(reg, tw_root_2n_mont[n2_idx * n1 + lane_id], mod, mont);
    }
}
...
__device__ void device_4step_ntt_forward_phase2_ws(BasicInteger* g_out,
                                                   ...
                                                   const BasicInteger* s_tw_root_n2_mont, BasicInteger mod,
                                                   BasicInteger mont) {
    const size_t warpsPerBlock = blockDim.x / warpSize;
    const size_t warp_id = threadIdx.x >> 5;
    const size_t lane_id = threadIdx.x & 31;
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        ...
        for (size_t log_m = 0; log_m < 5; log_m++) {
            ...
            BasicInteger left = (1 - C) * (reg - reg_new) + reg_new; // C = 0 choose reg, C = 1 choose reg_new
            BasicInteger right = C * (reg - reg_new) + reg_new; // C = 0 choose reg_new, C = 1 choose reg
            ct_butterfly_mont(left, right, s_tw_root_n2_mont[w_idx], mod, mont);
            reg = (1 - C) * (left - right) + right;
        }
...
__device__ void device_4step_ntt_forward_phase1_naive(BasicInteger s_n1n2[n1][n2 + 1],
    ...
    for (size_t n2_idx = warp_id; n2_idx < n2; n2_idx += warpsPerBlock) {
        for (size_t log_m = 0; log_m < 6; log_m++) {
            ...
            ct_butterfly_mont(s_n1n2[butt_idx][n2_idx], s_n1n2[butt_idx + (1 << log_step)][n2_idx],
                              s_tw_root_2n1_mont[(1 << log_m) + w_idx], mod, mont);
        }
        // elementwise multiply and transpose store into shared memory
        s_n1n2[lane_id][n2_idx] = modMulMontLazy(s_n1n2[lane_id][n2_idx],
                                                 tw_root_2n_mont[n2_idx * n1 + lane_id], mod, mont);
        s_n1n2[lane_id + warpSize][n2_idx] = modMulMontLazy(s_n1n2[lane_id + warpSize][n2_idx],
                                                            tw_root_2n_mont[n2_idx * n1 + lane_id + warpSize], mod,
                                                            mont);
    }
}
...
__device__ void device_4step_ntt_forward_phase2_naive(BasicInteger* g_out,
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        for (size_t log_m = 0; log_m < 6; log_m++) {
            ...
            size_t w_idx = lane_id >> log_step;
            size_t butt_idx = ((w_idx << log_step) + lane_id) & 0x3f;
            ct_butterfly_mont(s_in[n1_idx][butt_idx], s_in[n1_idx][butt_idx + (1 << log_step)],
                              s_tw_root_n2_mont[w_idx], mod, mont);
        }
        // reduce and write back to global memory
...
__device__ void device_4step_ntt_inverse_phase1_ws(BasicInteger s_n1n2[n1][n2 + 1],
                                                   ...
                                                   const BasicInteger* s_tw_inv_root_n2_mont,
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        ...
        for (int log_m = 4; log_m >= 0; log_m--) {
            ...
            BasicInteger left = (1 - C) * (reg - reg_new) + reg_new; // C = 0 choose reg, C = 1 choose reg_new
            BasicInteger right = C * (reg - reg_new) + reg_new; // C = 0 choose reg_new, C = 1 choose reg
            gs_butterfly_mont(left, right, s_tw_inv_root_n2_mont[w_idx], mod, mont);
            reg = (1 - C) * (left - right) + right;
        }
        // elementwise multiply and transpose store into shared memory
        s_n1n2[n1_idx][lane_id] = modMulMontLazy(reg, tw_inv_root_2n_mont[n1_idx * n2 + lane_id], mod, mont);
    }
}

template<size_t n1, size_t n2>
__device__ void device_4step_ntt_inverse_phase2_ws(BasicInteger s_n1n2[n1][n2 + 1],
                                                   const BasicInteger* s_tw_inv_root_2n1_mont,
                                                   BasicInteger inv_n_mont,
                                                   BasicInteger mod, BasicInteger mont) {
    const size_t warpsPerBlock = blockDim.x / warpSize;
    const size_t warp_id = threadIdx.x >> 5;
    const size_t lane_id = threadIdx.x & 31;
    ...
    for (size_t n2_idx = warp_id; n2_idx < n2; n2_idx += warpsPerBlock) {
        ...
        for (int log_m = 4; log_m >= 0; log_m--) {
            ...
            BasicInteger left = (1 - C) * (reg - reg_new) + reg_new; // C = 0 choose reg, C = 1 choose reg_new
            BasicInteger right = C * (reg - reg_new) + reg_new; // C = 0 choose reg_new, C = 1 choose reg
            gs_butterfly_mont(left, right, s_tw_inv_root_2n1_mont[(1 << log_m) + w_idx], mod, mont);
            reg = (1 - C) * (left - right) + right;
        }
        // transpose write back to s_out
        s_n1n2[lane_id][n2_idx] = modMulMont(reg, inv_n_mont, mod, mont);
    }
}
...
__device__ void device_4step_ntt_inverse_phase1_naive(BasicInteger s_n1n2[n1][n2 + 1],
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        for (int log_m = 5; log_m >= 0; log_m--) {
            ...
            size_t w_idx = lane_id >> log_step;
            size_t butt_idx = ((w_idx << log_step) + lane_id) & 0x3f;
            gs_butterfly_mont(s_n1n2[n1_idx][butt_idx], s_n1n2[n1_idx][butt_idx + (1 << log_step)],
                              s_tw_inv_root_n2_mont[w_idx], mod, mont);
        }
        // elementwise multiply and transpose store into shared memory
        s_n1n2[n1_idx][lane_id] = modMulMontLazy(s_n1n2[n1_idx][lane_id],
                                                 tw_inv_root_2n_mont[n1_idx * n2 + lane_id],
                                                 mod, mont);
        s_n1n2[n1_idx][lane_id + warpSize] = modMulMontLazy(s_n1n2[n1_idx][lane_id + warpSize],
                                                            tw_inv_root_2n_mont[n1_idx * n2 + lane_id + warpSize],
                                                            mod, mont);
    }
}
...
__device__ void device_4step_ntt_inverse_phase2_naive(BasicInteger s_n1n2[n1][n2 + 1],
    ...
    for (size_t n2_idx = warp_id; n2_idx < n2; n2_idx += warpsPerBlock) {
        for (int log_m = 5; log_m >= 0; log_m--) {
            ...
            size_t w_idx = lane_id >> log_step;
            size_t butt_idx = ((w_idx << log_step) + lane_id) & 0x3f;
            gs_butterfly_mont(s_n1n2[butt_idx][n2_idx], s_n1n2[butt_idx + (1 << log_step)][n2_idx],
                              s_tw_inv_root_2n1_mont[(1 << log_m) + w_idx],
                              mod, mont);
        }
        // transpose write back to s_out
        BasicInteger tmp = modMulMontLazy(s_n1n2[lane_id][n2_idx], inv_n_mont, mod, mont);
        s_n1n2[lane_id][n2_idx] = modFast(tmp, mod);
        tmp = modMulMontLazy(s_n1n2[lane_id + warpSize][n2_idx], inv_n_mont, mod, mont);
