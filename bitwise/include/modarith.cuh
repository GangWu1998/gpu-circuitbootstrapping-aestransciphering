...
__device__ wide_type<T> mulWide(const T op1, const T op2) {
    wide_type<T> res;
    res.lo = op1 * op2;
...
template<typename T>
    requires supportedTypes<T>
__host__ __device__ T modFast(const T op, const T mod) {
    const T tmp = op - mod;
...
template<typename T>
    requires supportedTypes<T>
__device__ T modBarrett(const T op, const T mod, const T mu) {
    T s = mulHi(op, mu);
    T res = op - s * mod;
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMont(const T op, const T mod, const T mu) {
    const T t = op * mu;
    const T res = mod - mulHi(t, mod);
    return modFast(res, mod);
}
...
/**
 * Lazy Montgomery reduction for double-width operand and modulus
 * @param op
 * @param mod
 * @param mu Montgomery constant
 * @return reduced op in [0, 2*mod]
 */
__device__ inline uint64_t modWideMontLazy(const wide_type<uint64_t> op, const uint64_t mod, const uint64_t mu) {
    uint64_t res;
    ...
    asm(
        ...
        "}"
        : "=l"(res)
        : "l"(op.lo), "l"(op.hi), "l"(mod), "l"(mu));

    return res;
}

__device__ inline uint32_t modWideMontLazy(const wide_type<uint32_t> op, const uint32_t mod, const uint32_t mu) {
    uint32_t res;
    ...
    asm(
        ...
        "}"
        : "=r"(res)
        : "r"(op.lo), "r"(op.hi), "r"(mod), "r"(mu));

    return res;
}

/**
 * Montgomery reduction for double-width operand and modulus
 * @param op
 * @param mod
 * @param mu Montgomery constant
 * @return reduced op in [0, mod]
 */
__device__ inline uint64_t modWideMont(const wide_type<uint64_t> op, const uint64_t mod, const uint64_t mu) {
    uint64_t res;
...
__device__ inline uint32_t modWideMont(const wide_type<uint32_t> op, const uint32_t mod, const uint32_t mu) {
    uint32_t res;
...
template<typename T>
    requires supportedTypes<T>
__host__ __device__ T modAdd(const T op1, const T op2, const T mod) {
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMulBarrett(const T op1, const T op2, const T mod, const wide_type<T> mu) {
    return modWideBarrett(mulWide(op1, op2), mod, mu);
}
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMulShoupLazy(const T op1, const T op2, const T op2_shoup, const T mod) {
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMulShoup(const T op1, const T op2, const T op2_shoup, const T mod) {
    T hi = mulHi(op1, op2_shoup);
...
/**
 * Modular multiplication using wide Montgomery reduction
 * @param op1
 * @param op2
 * @param mod
 * @param mu Montgomery constant
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMulMont(const T op1, const T op2, const T mod, const T mu) {
    return modWideMont(mulWide(op1, op2), mod, mu);
}

/**
 * Lazy Modular multiplication using wide Montgomery reduction
 * @param op1
 * @param op2
 * @param mod
 * @param mu Montgomery constant
...
template<typename T>
    requires supportedTypes<T>
__device__ T modMulMontLazy(const T op1, const T op2, const T mod, const T mu) {
    return modWideMontLazy(mulWide(op1, op2), mod, mu);
}

template<typename T>
    requires supportedTypes<T>
__device__ T toMont(const T& coeff, const T& R2_mod_q, const T& q, const T& q_inv_mod_R) {
    return modMulMont(coeff, R2_mod_q, q, q_inv_mod_R);
}

template<typename T>
    requires supportedTypes<T>
__device__ T fromMont(const T& coeff, const T& q, const T& q_inv_mod_R) {
    return modMont(coeff, q, q_inv_mod_R);
}
