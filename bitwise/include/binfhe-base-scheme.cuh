...
namespace phantom::bitwise
{
    ...
    private:
         ...
         * @param EK a shared pointer to the bootstrapping keys
         * @param ctprep input ciphertext
         * @param use_fft flag to indicate if FFT-based blind rotation is used
         * @param s CUDA stream
         * @return the output RingLWE accumulator
         */
    ...
    public:
        explicit GPUBinFHEScheme(const std::shared_ptr<lbcrypto::BinFHECryptoParams>& params, const cudaStream_t& s)
        {
            ...
            GPUACCscheme_ = std::make_shared<GPURingGSWAccumulator>(true);
        }
        ...
        std::vector<lbcrypto::LWECiphertext>
        BatchGPUEvalFloor(const std::shared_ptr<lbcrypto::BinFHECryptoParams>& params,
                          const GPURingGSWBTKey& EK,
                          const std::vector<lbcrypto::LWECiphertext>& v_ct,
        ...
        std::vector<lbcrypto::LWECiphertext>
        BatchGPUEvalSign(const std::shared_ptr<lbcrypto::BinFHECryptoParams>& params,
         ...
         */
        std::vector<lbcrypto::LWECiphertext>
        GPUEvalDecomp(const std::shared_ptr<lbcrypto::BinFHECryptoParams>& params,
                      const std::map<uint32_t, GPURingGSWBTKey>& EKs,
                      lbcrypto::ConstLWECiphertext& ct,
                      const NativeInteger& beta,
                      const cudaStream_t& s) const;
        ...
        BatchGPUEvalDecomp(const std::shared_ptr<lbcrypto::BinFHECryptoParams>& params,
                           const std::map<uint32_t, GPURingGSWBTKey>& EKs,
                           const std::vector<lbcrypto::LWECiphertext>& v_ct,
                           const NativeInteger& beta,
                           const cudaStream_t& s) const;
