...
__global__ void kernel_EvalAccCoreDM_1024_batch_fuse(
    ...
    const size_t N, const BasicInteger mod, const BasicInteger mont, const size_t digitsG2,
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        ...
        BasicInteger reg = modMulMontLazy(dct_first, acc_key[poly_idx * N + N_idx], mod, mont);
        for (size_t digitsG2_index = 1; digitsG2_index < digitsG2; ++digitsG2_index) {
            ...
            reg += modMulMontLazy(dct_coeff, acc_key[digitsG2_index * 2 * N + poly_idx * N + N_idx], mod, mont);
            reg = modFast(reg, mod2);
        }
...
__global__ void kernel_EvalAccCoreCGGI_1024_batch_fuse(
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        ...
        BasicInteger dct_first = dct[batch_idx * digitsG2 * N + N_idx];
        BasicInteger tmp0 = modMulMontLazy(dct_first, acc_key0[poly_idx * N + N_idx], mod, mont);
        BasicInteger tmp1 = modMulMontLazy(dct_first, acc_key1[poly_idx * N + N_idx], mod, mont);
        ...
        for (uint32_t digitsG2_idx = 1; digitsG2_idx < digitsG2; ++digitsG2_idx) {
            ...
            tmp0 += modMulMontLazy(dct_coeff, acc_key0[digitsG2_idx * 2 * N + poly_idx * N + N_idx], mod, mont);
            tmp0 = modFast(tmp0, mod2);

            tmp1 += modMulMontLazy(dct_coeff, acc_key1[digitsG2_idx * 2 * N + poly_idx * N + N_idx], mod, mont);
            tmp1 = modFast(tmp1, mod2);
        }

        wide_type<BasicInteger> acc_wide = addWide(mulWide(tmp0, pos_monic_coeff),
                                                   mulWide(tmp1, neg_monic_coeff));

        BasicInteger reg = modWideMontLazy(acc_wide, mod, mont);
...
__global__ void kernel_EvalAccCoreCGGI_1024_binary_batch_fuse(
    ...
    for (size_t n1_idx = warp_id; n1_idx < n1; n1_idx += warpsPerBlock) {
        ...
        for (uint32_t digitsG2_idx = 1; digitsG2_idx < digitsG2; ++digitsG2_idx) {
            ...
            tmp += modMulMontLazy(dct_coeff, acc_key[digitsG2_idx * 2 * N + poly_idx * N + N_idx], mod, mont);
            tmp = modFast(tmp, mod2);
        }

        BasicInteger reg = modMulMontLazy(tmp, pos_monic_coeff, mod, mont);
