...
namespace phantom::bitwise
{
                                               ...
                                               KEYGEN_MODE keygenMode) const
    {
        ...
        if (RGSWParams->IsCompositeNTT())
        {
            ...
            GPUACCscheme_->GPUKeyGenAcc(gpu_ek, RGSWParams, new_skN, LWEsk, s);
        }
        else
        {
            GPUACCscheme_->GPUKeyGenAcc(gpu_ek, RGSWParams, skN->GetElement(), LWEsk, s);
        }
                                          ...
                                          const cudaStream_t& s) const
    {
        ...
        GPUACCscheme_->GPUEvalAcc(params, EK, ctprep->GetA(), d_acc, use_fft, s);
                                                  ...
                                                  const cudaStream_t& s) const
    {
        ...
        auto d_acc = GPUBootstrapGateCore(params, gate, EK, ctprep, true, s);
                                          ...
                                          const cudaStream_t& s) const
    {
        ...
        // main accumulation computation
        // the following loop is the bottleneck of bootstrapping/binary gate evaluation
        GPUACCscheme_->GPUEvalAcc(params, EK, ct->GetA(), d_acc, use_fft, s);
    ...
    LWECiphertext GPUBinFHEScheme::GPUBootstrapFunc(const std::shared_ptr<BinFHECryptoParams>& params,
                                                    ...
                                                    const cudaStream_t& s) const
    {
        auto d_acc = GPUBootstrapFuncCore(params, EK, ct, f, fmod, false, s);
    ...
    LWECiphertext GPUBinFHEScheme::GPUEvalSign(const std::shared_ptr<BinFHECryptoParams>& params,
    ...
    std::vector<LWECiphertext> GPUBinFHEScheme::GPUEvalDecomp(const std::shared_ptr<BinFHECryptoParams>& params,
                                                              const std::map<uint32_t, GPURingGSWBTKey>& EKs,
                                                              ConstLWECiphertext& ct, const NativeInteger& beta,
                                                              const cudaStream_t& s) const
    {
