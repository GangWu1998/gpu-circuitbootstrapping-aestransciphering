...
namespace phantom::bitwise {
    ...
    GPUBinFHEScheme::BatchGPUBootstrapGateCore(const std::shared_ptr<BinFHECryptoParams>& params,
        ...
        const NativeInteger Q = RGSWParams->GetQ();
        const NativeInteger Q2p = Q / NativeInteger(2 * p) + 1;
        const NativeInteger Q2pNeg = Q - Q2p;

        const uint32_t N = LWEParams->GetN();

        // Since q | (2*N), we deal with a sparse embedding of Z_Q[x]/(X^{q/2}+1) to
        // Z_Q[x]/(X^N+1)
        const uint32_t factor = (2 * N / q.ConvertToInt());

        const size_t batch_size = v_ctprep.size();

        std::vector v_m(batch_size, NativeVector(N, Q));
        ...
        GPUACCscheme_->BatchGPUEvalAcc(params, EK, v_ctprep_A, d_acc_batch, true, s);
    ...
    std::vector<LWECiphertext> GPUBinFHEScheme::BatchGPUEvalBinGate(const std::shared_ptr<BinFHECryptoParams>& params,
        ...
        const size_t batch_size = v_ct1.size();

        std::vector<LWECiphertext> v_ctprep;

        for (size_t i = 0; i < batch_size; ++i) {
            auto ctprep = std::make_shared<LWECiphertextImpl>(*v_ct1[i]);
        ...
        std::vector v_A(batch_size, NativeVector(n, v_ct1[0]->GetModulus()));
        for (size_t i = 0; i < batch_size; ++i) {
    ...
    GPUBinFHEScheme::BatchGPUBootstrapFuncCore(const std::shared_ptr<BinFHECryptoParams>& params,
        ...
        GPUACCscheme_->BatchGPUEvalAcc(params, EK, v_ct_A, d_acc_batch, false, s);
    ...
    std::vector<LWECiphertext> GPUBinFHEScheme::BatchGPUBootstrapFunc(const std::shared_ptr<BinFHECryptoParams>& params,
                                                                      ...
                                                                      const NativeInteger& fmod,
                                                                      const cudaStream_t& s) const {
        auto d_acc_batch = BatchGPUBootstrapFuncCore(params, EK, v_ct, f, fmod, s);
    ...
    std::vector<LWECiphertext> GPUBinFHEScheme::BatchGPUEvalFunc(const std::shared_ptr<BinFHECryptoParams>& params,
        ...
        if (functionProperty == 0) {
            // negacyclic function only needs one bootstrap
        ...
        if (functionProperty == 2) {
            // arbitrary function
            const auto& LWEParams = params->GetLWEParams();
            uint32_t N{LWEParams->GetN()};
            if (q.ConvertToInt() > N) {
                // need q to be at most = N for arbitrary function
                std::string errMsg =
    ...
    std::vector<LWECiphertext> GPUBinFHEScheme::BatchGPUEvalFloor(const std::shared_ptr<BinFHECryptoParams>& params,
                                                                  const GPURingGSWBTKey& EK,
                                                                  const std::vector<LWECiphertext>& v_ct,
                                                                  const NativeInteger& beta,
        ...
        NativeInteger mod{v_ct[0]->GetModulus()};

        for (size_t i = 0; i < batch_size; ++i)
            LWEscheme->EvalAddConstEq(v_ct1[i], beta);

        std::vector<LWECiphertext> v_ct1Modq;
        for (size_t i = 0; i < batch_size; ++i) {
            LWECiphertext ct1Modq = std::make_shared<LWECiphertextImpl>(*v_ct1[i]);
            v_ct1Modq.push_back(ct1Modq);
        }

        for (size_t i = 0; i < batch_size; ++i)
            v_ct1Modq[i]->SetModulus(q);
        // this is 1/4q_small or -1/4q_small mod q
        ...
        auto v_ct2 = BatchGPUBootstrapFunc(params, EK, v_ct1Modq, f1, mod, s);
        for (size_t i = 0; i < batch_size; ++i)
            LWEscheme->EvalSubEq(v_ct1[i], v_ct2[i]);

        std::vector<LWECiphertext> v_ct2Modq;
        for (size_t i = 0; i < batch_size; ++i) {
            LWECiphertext ct2Modq = std::make_shared<LWECiphertextImpl>(*v_ct1[i]);
            v_ct2Modq.push_back(ct2Modq);
        }

        for (size_t i = 0; i < batch_size; ++i)
            v_ct2Modq[i]->SetModulus(q);
        ...
        auto v_ct3 = BatchGPUBootstrapFunc(params, EK, v_ct2Modq, f2, mod, s);
        for (size_t i = 0; i < batch_size; ++i)
            LWEscheme->EvalSubEq(v_ct1[i], v_ct3[i]);

        return v_ct1;
    }

    std::vector<LWECiphertext> GPUBinFHEScheme::BatchGPUEvalSign(const std::shared_ptr<BinFHECryptoParams>& params,
                                                                 const std::map<uint32_t, GPURingGSWBTKey>& EKs,
                                                                 const std::vector<LWECiphertext>& v_ct,
                                                                 const NativeInteger& beta,
                                                                 const cudaStream_t& s, bool schemeSwitch) const {
        const size_t batch_size = v_ct.size();

        auto mod{v_ct[0]->GetModulus()};
        const auto& LWEParams = params->GetLWEParams();
        ...
        std::vector<LWECiphertext> v_cttmp;
        for (size_t i = 0; i < batch_size; ++i) {
            LWECiphertext cttmp = std::make_shared<LWECiphertextImpl>(*v_ct[i]);
            v_cttmp.push_back(cttmp);
        }
        while (mod > q) {
            v_cttmp = BatchGPUEvalFloor(params, curEK, v_cttmp, beta, s);
            // round Q to 2betaQ/q
            //  mod   = mod / q * 2 * beta;
            mod = (mod << 1) * beta / q;
            for (auto &cttmp : v_cttmp) {
        ...
        if (!schemeSwitch) {
            ...
            v_cttmp = BatchGPUBootstrapFunc(params, curEK, v_cttmp, f3, q, s); // this is 1/4q_small or -1/4q_small mod q
            for (auto &cttmp : v_cttmp) {
                LWEscheme->EvalSubConstEq(cttmp, q >> 2);
            }
        } else {
            // return the negated f3 and do not subtract q/4 for a more natural encoding in scheme switching
            // if the ended q is smaller than q, we need to change the param for the final boostrapping
            auto f3 = [](NativeInteger x, NativeInteger q, NativeInteger Q) -> NativeInteger {
    ...
    std::vector<std::vector<LWECiphertext>>
    GPUBinFHEScheme::BatchGPUEvalDecomp(const std::shared_ptr<BinFHECryptoParams>& params,
                                        const std::map<uint32_t, GPURingGSWBTKey>& EKs,
                                        const std::vector<LWECiphertext>& v_ct,
                                        const NativeInteger& beta,
                                        const cudaStream_t& s) const {
        ...
        std::vector<std::vector<LWECiphertext>> v_ret(batch_size);
        while (mod > q) {
            std::vector<LWECiphertext> v_ctq;
            for (size_t i = 0; i < batch_size; ++i) {
                LWECiphertext ctq = std::make_shared<LWECiphertextImpl>(*v_cttmp[i]);
                v_ctq.push_back(ctq);
            }
            for (size_t i = 0; i < batch_size; ++i) {
                v_ctq[i]->SetModulus(q);
                v_ret[i].push_back(std::move(v_ctq[i]));
            }
        ...
        RGSWParams->Change_BaseG(curBase);
        for (size_t i = 0; i < batch_size; ++i) {
            v_ret[i].push_back(std::move(v_cttmp[i]));
        }
        return v_ret;
    }
}
